<div align="center">
  ...
  <p>
    <img src="https://img.shields.io/badge/version-4.0.0-blue?style=flat" alt="Version" />
...
#### 📊 CSV插值模式（四步流程）
1. **📁 选择文件** - 上传需要插值的CSV文件（支持拖拽）
2. **⚙️ 配置参数** - 设置插值距离、速度、海拔、插值范围等参数
3. **▶️ 开始处理** - 点击处理按钮，系统自动进行插值处理
4. **💾 下载结果** - 查看处理统计并下载插值后的CSV文件
...
1. **解析CSV**：读取CSV文件并解析轨迹点数据
2. **范围过滤**：根据配置过滤前部和后部轨迹点（可选）
3. **智能插值**：在中间部分的轨迹点之间插入新点
4. **生成结果**：输出插值后的CSV文件

**输出文件**：
- 文件名格式：`原文件名_interpolated.csv`
- 文件格式：标准一生足迹CSV格式，包含插值后的轨迹数据
